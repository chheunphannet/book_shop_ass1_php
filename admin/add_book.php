...
try {
    ...
    $newBookId = $service->createBookByCategoryName(
        $title,
        $categoryName,
        $pageNumber,
        $unitPrice,
    ...
    if ($newBookId === null) {
        ...
        echo json_encode(['ok' => false, 'message' => 'Failed to add book']);
        exit;
    }

    echo json_encode(['ok' => true, 'message' => 'Book created successfully', 'book_id' => $newBookId]);
} catch (Throwable $e) {
